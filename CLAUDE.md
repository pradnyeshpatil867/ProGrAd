...
**Retrieval is two-stage**, not a single similarity search: [qdrant_service.py](app/services/retrieval/qdrant_service.py) pulls 15 dense-vector candidates from Qdrant, then [ranking_service.py](app/services/retrieval/ranking_service.py) reranks with a local FlashRank cross-encoder down to the top 5. There is no BM25/sparse component today — retrieval is vectors-only.
